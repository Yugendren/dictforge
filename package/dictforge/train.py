#!/usr/bin/env python3
"""
dictforge/train.py -- validation-driven zstd dictionary trainer.
...
Pipeline:
  ...
  2. Stage 2 (target_level >= 16 only): iteratively evict poorly-
     referenced 64-byte buckets (measured via the refine_dict native
     helper's coverage mode) and refill the freed space with excerpts
     from the worst-compressing fit files, accepting a round only if it
     shrinks the fit-set's batch-compressed size.
  3. Stage 3 (target_level >= 16 only): reseed the dictionary's repeat-
     offset codes from the offset_hist native helper's top offsets,
     keeping the patch only if it improves the held-out val ratio.
...
recording every decision made along the way. Output is a 100%-standard
zstd dictionary -- usable by any stock zstd, in any language. dictforge
is a training-time tool only.

Binary resolution:
  zstd    -- resolved from PATH by default; override with --zstd-bin or
             $DICTFORGE_ZSTD_BIN.
  native
  helpers -- resolved from <package>/native/bin by default (build them
             first with build_native.sh); override the directory with
             --native-dir or $DICTFORGE_NATIVE_DIR. Only required for
             --target-level >= 16 (stages 2/3); stage-1-only runs
             (--stages 1) never need them.

CLI:
    train.py --train-dir DIR --out DICT --target-level N
             [--max-budget BYTES] [--time-budget-s SECONDS]
             [--seed SEED] [--stages all|1]
             [--zstd-bin PATH] [--native-dir DIR]
"""
import argparse
import json
import os
...
try:
    from .patch_repcodes import patch_repcodes, find_repcode_field
except ImportError:
    # allow `python3 train.py` (uninstalled, run directly out of the
    # package directory) as well as `python3 -m dictforge.train`
...
# Resolved lazily in main(); native helper paths are only required for
# --target-level >= 16, so we don't hard-fail on missing binaries until
# a stage that actually needs them runs.
REFINE_DICT_BIN = None
OFFSET_HIST_BIN = None


def log(msg):
    print(f"[dictforge] {msg}", flush=True)
...
def resolve_zstd_bin(cli_value):
    """Resolve the zstd binary to use: --zstd-bin, then
    $DICTFORGE_ZSTD_BIN, then PATH. Errors out with a clear message if
    none of those produce a usable binary."""
    candidates = []
    if cli_value:
        candidates.append(("--zstd-bin", cli_value))
    env_val = os.environ.get("DICTFORGE_ZSTD_BIN")
    if env_val:
        candidates.append(("$DICTFORGE_ZSTD_BIN", env_val))

    for source, path in candidates:
        if os.path.isfile(path) and os.access(path, os.X_OK):
            return path
        log(f"WARNING: {source}={path!r} is not an executable file; ignoring")
    ...
    sys.exit(
        "dictforge: could not find a zstd binary.\n"
        "  Tried (in order): --zstd-bin, $DICTFORGE_ZSTD_BIN, PATH.\n"
        "  Install zstd (e.g. `brew install zstd` / `apt install zstd`) so "
        "it's on PATH, or pass --zstd-bin /path/to/zstd."
    )


def resolve_native_dir(cli_value):
    """Resolve the directory containing the refine_dict / offset_hist
    native helper binaries: --native-dir, then $DICTFORGE_NATIVE_DIR,
    then <package>/native/bin."""
    if cli_value:
        return Path(cli_value)
    env_val = os.environ.get("DICTFORGE_NATIVE_DIR")
    if env_val:
        return Path(env_val)
    return HERE / "native" / "bin"


def require_native_helpers(native_dir):
    """Set REFINE_DICT_BIN / OFFSET_HIST_BIN or exit with a clear error
    pointing at build_native.sh. Only called when target_level >= 16."""
    global REFINE_DICT_BIN, OFFSET_HIST_BIN
    refine = Path(native_dir) / "refine_dict"
    offset = Path(native_dir) / "offset_hist"
    missing = [str(p) for p in (refine, offset) if not p.is_file()]
    if missing:
        sys.exit(
            "dictforge: native helper binaries not found (needed for "
            "--target-level >= 16, which runs stages 2/3):\n"
            + "\n".join(f"  missing: {m}" for m in missing)
            + f"\n  Looked in: {native_dir}\n"
            "  Build them with: ./build_native.sh\n"
            "  (or point at a prebuilt directory with --native-dir / "
            "$DICTFORGE_NATIVE_DIR)\n"
            "  To skip stages 2/3 entirely, pass --stages 1 (works with "
            "no native helpers)."
        )
    REFINE_DICT_BIN, OFFSET_HIST_BIN = refine, offset
...
def main():
    ...
    ap.add_argument("--zstd-bin", default=None,
                     help="path to zstd binary (default: $DICTFORGE_ZSTD_BIN, "
                          "then PATH)")
    ap.add_argument("--native-dir", default=None,
                     help="directory containing refine_dict/offset_hist "
                          "(default: $DICTFORGE_NATIVE_DIR, then "
                          "<package>/native/bin); only needed for "
                          "--target-level >= 16")
    args = ap.parse_args()

    zstd_bin = resolve_zstd_bin(args.zstd_bin)
    native_dir = resolve_native_dir(args.native_dir)
    if args.target_level >= 16 and args.stages == "all":
        require_native_helpers(native_dir)
    else:
        log(f"native helpers not required for this run "
            f"(target_level={args.target_level}, stages={args.stages}); "
            f"skipping native-dir check ({native_dir})")
    ...
    workdir = Path(subprocess.run(
        ["mktemp", "-d", "/tmp/dictforge_train.XXXXXX"],
